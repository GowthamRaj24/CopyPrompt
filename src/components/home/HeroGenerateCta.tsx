 ...
 * a rotating multi-color conic-gradient halo (blue → purple → pink →
 * yellow → green), a 4-point Gemini star icon with the same gradient,
 * gradient-painted text that shimmers left-to-right, and an arrow that
 * shifts right on hover. The whole thing scales by 1.5% on hover for
 * the tiniest bit of physicality.
 *
...
export function HeroGenerateCta() {
  return (
    <div className="hero-gen-wrap relative inline-block">
      {/* Rotating gradient halo behind the pill */}
      <span aria-hidden className="hero-gen-halo" />

      <Link
        href="/generate"
        aria-label="Generate your own AI prompt with Gemini"
        className="hero-gen-pill press group relative inline-flex items-center gap-2.5 rounded-full border border-primary/30 bg-card/85 px-4 py-2 text-[13.5px] font-semibold backdrop-blur-md transition-all duration-300 hover:scale-[1.025] hover:border-primary/50 sm:gap-3 sm:px-5 sm:py-2.5"
      >
